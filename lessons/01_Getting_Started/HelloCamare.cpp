    ...
    while (!glfwWindowShouldClose(window))
    {
        ...
        for (unsigned int i = 0; i < 10; i++)
        {
            ...
            model = glm::translate(model, cubePositions[i]);
             model = glm::rotate(model, (float)glfwGetTime() * glm::radians(100.0f * sin(glm::length(cubePositions[i]) + 1)), glm::vec3(1.0f, 0.3f, 0.5f));
            // model = glm::scale(model, sin((float)glfwGetTime() + glm::length(cubePositions[i])) * glm::vec3(0.5f) );
            ...
            glm::mat4 view = glm::mat4(1.0f);
            view = glm::lookAt(glm::vec3(camX, 0.0, camZ), glm::vec3(0.0, 0.0, 0.0), glm::vec3(1.0, 1.0, 0.0));
